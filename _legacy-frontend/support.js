...
(() => {
  ...
  function walkText(node) {
    ...
    // HTML <span> is not rendered inside SVG text content (e.g. <text>{{name}}</text>
    // draws nothing); interpolations there must be SVG <tspan> instead.
    const svgNS = "http://www.w3.org/2000/svg";
    const p0 = node.parentElement;
    const interpTag = p0 && p0.namespaceURI === svgNS && p0.tagName.toLowerCase() !== "foreignobject" ? "tspan" : "span";
