/* GitSim — client-side Git simulation + commit-graph layout.
   Isolated module: no DOM, no framework. window.GitSim = { seed, exec, layout }.
   Swap point for a future backend: replace exec() with an API call returning the same shape. */
(function () {
  ...
  window.GitSim = { seed: seed, exec: exec, layout: layout, headTip: headTip };
})();
